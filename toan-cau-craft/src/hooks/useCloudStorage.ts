...
export const useCloudStorage = () => {
    const handleUploadImage = async ({ images }: HandleUploadImageProps): Promise<ImageProduct[]> => {
        const imageProductArray = [];
        for (let i = 0; i < images.length; i++) {
            try {
                const imageProduct = await uploadImage(images[i], i);
    ...
    const uploadImage = (image: ExportDataImage): Promise<ImageProduct> => { 
    ...
    const deleteImage = (imageUrl: string): Promise<void> => {
    ...
    return {handleUploadImage, uploadImage, deleteImage}
}
